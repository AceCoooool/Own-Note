...
### ① Demo1：nms_gpu（没有写反向传播过程）

![](png/a1.png)

而每个Block"做什么呢"？（针对第i行，第j列的Block），其中的每个线程我们可以"看做行"（那么就能"凑齐"N行了）那么每个线程(idx)计算的是当前位置$64\cdot i+idx$与$[64\cdot j,64\cdot(j+1)$]这64个boxes对应的IoU情况 --- 这里采用一个`unsigned long long`来记录这64个IoU是否超过了给定的阈值（`1<<k`代表第k+1个boxes与当前位置的box重叠率超过阈值）

> 这部分输出是采用一个大小为$N\times (N/64)$的mask_dev来记录：
>
> ![](png/a2.png)
>
> 其中每个$N/64$的块可以看成某个box和其他所有boxes的重叠情况。因为$N/64$中的每个值都代表当前box和64个其他boxes之间的重叠率。
